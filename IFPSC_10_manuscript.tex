...
%	
	...
	\begin{table}[htb!]
		\caption{Simulation results for Potoff, MCMC-nb, and MCMC-nb-tors. Subscripts and superscripts (when present) correspond to the lower and upper 95~\% confidence intervals, respectively. If only a subscript is present the lower and upper uncertainties are nearly equal (i.e., the distribution of bootstrap samples is essentially symmetric) and only the average uncertainty is reported.} \label{tab:tabulated_values}
		\begin{center}
...
%				$T$ (K) & 	$\rho$ (kg/m$^3$) & 	$P$ (MPa) & 	$\eta_{\rm Potoff}$ (mPa-s) & 	$\eta_{\rm MCMC-nb}$ (mPa-s) & 	$\eta_{\rm MCMC-nb-tors}$ (mPa-s) \\ \hline
	...
	\begin{figure}[htb!]
		\centering
		\includegraphics[width=3.1in]{viscosity_pressure_results.pdf}
