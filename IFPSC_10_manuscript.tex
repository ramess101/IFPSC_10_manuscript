...
%	Historically, non-equilibrium molecular dynamics (NEMD) has been preferred for highly viscous systems \cite{McCabe2001,Liu2015}. However, in our recent publication we successfully predicted the viscosity of 2,2,4-trimethylpentane at 293 K and 1000 MPa (the highest pressure required for the challenge) with equilibrium molecular dynamics (EMD) \cite{Postdoc_3}. 

%Although non-equilibrium molecular dynamics (NEMD) is a common approach for simulating highly viscous systems \cite{McCabe2001,Liu2015}, we utilize equilibrium molecular dynamics (EMD) in this study. In our recent publication, we successfully implemented EMD to predict the viscosity of 2,2,4-trimethylpentane at 293 K and 1000 MPa (the highest pressure required for the challenge) \cite{Postdoc_3}.
...
%    For 224TMH, the CH$_3$ parameters contribute to the majority of non-bonded interactions (both between different molecules as well as the 1-5 and 1-6 interactions within the same molecule). Therefore, the relatively small uncertainties assigned to the CH$_3$ parameters (recall Figure \ref{fig:nonbonded_uncertainty}) are likely the cause for the negligible impact of non-bonded uncertainties.
    ...
    %Due to the negligible contribution of non-bonded and torsional uncertainties, we did not investigate other sources of parameter uncertainty, e.g., the bond lengths, equilibrium bond angles, and angular harmonic force constants. 
